...
# -------------------------------------------------------------------------------
# Role:: nomad-client-02
#
# Per-node config for nomad-client-02 (192.168.68.72) -- a proxmox VM
# running as a nomad+consul client.
# -------------------------------------------------------------------------------

name 'nomad-client-02'
description 'nomad-client-02: cluster nomad+consul client on a proxmox VM'
...
default_attributes(
  ...
  global: {
    dns_endpoint_ip: '192.168.68.72',
  },
  consul: {
    config: {
      bind_addr: '192.168.68.72',
    },
  },
  nomad: {
    config: {
      node_name:    'nomad-client-02',
      advertise_ip: '192.168.68.72',
    },
  },
  vault_cert_manager: {
    certificates: [
      {
        ...
        ttl:          '720h',
        alt_names:    %w(localhost nomad-client-02 nomad-client-02.munchbox.cc client.munchbox.consul),
        ip_sans:      %w(192.168.68.72 127.0.0.1),
        owner:        'consul',
        group:        'consul',
      ...
      },
      {
        ...
        ttl:          '720h',
        alt_names:    %w(localhost nomad-client-02 nomad-client-02.munchbox.cc client.global.nomad),
        ip_sans:      %w(192.168.68.72 127.0.0.1),
        owner:        'nomad',
        group:        'nomad',
        on_change:    'systemctl restart nomad',
        health_check: { tcp: '192.168.68.72:4646', timeout: '5s' },
      },
    ],
  }
)
