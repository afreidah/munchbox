...
#
# One-shot pre-provision step for a new chef-managed node. Runs the
# three things that MUST exist on Vault + chef-server BEFORE the node
# boots and cloud-init tries to converge:
#
...
#   3. Upload the per-node chef role file from
#      infrastructure/cinc/roles/nodes/<node>.rb.
#
# After this completes, the bootstrap cloud-init can successfully fetch
# its vault-agent creds via the encrypted data bag on first chef converge.
#
# Usage:
...
#
# Idempotent -- safe to re-run if anything failed mid-way. The role
# upload is always a re-upload; the secret_id mint always generates a
# fresh value (and rotates the Vault KV entry as a side effect).
# -------------------------------------------------------------------------------
set -euo pipefail
...
ROLE_FILE_HYPHENATED="${SCRIPT_DIR}/../roles/nodes/$(echo "$NODE" | sed -E 's/([a-z]+)([0-9]+)/\1-\2/').rb"
ROLE_FILE_PLAIN="${SCRIPT_DIR}/../roles/nodes/${NODE}.rb"
...
# --- 3. Upload the per-node chef role file. ---
echo "[3/3] Uploading chef role for $NODE..."
if [[ -f "$ROLE_FILE_HYPHENATED" ]]; then
  ROLE_FILE="$ROLE_FILE_HYPHENATED"
elif [[ -f "$ROLE_FILE_PLAIN" ]]; then
  ROLE_FILE="$ROLE_FILE_PLAIN"
else
  echo "error: no role file found at $ROLE_FILE_HYPHENATED or $ROLE_FILE_PLAIN" >&2
  echo "       create the per-node role first, then re-run." >&2
  exit 1
fi
knife role from file "$ROLE_FILE"
echo "      uploaded $ROLE_FILE"
