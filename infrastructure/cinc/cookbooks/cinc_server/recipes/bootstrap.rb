...
bootstrap['extra_users'].each do |extra|
  creds = paths['extra_user_credentials'][extra['username']]

  cinc_server_user extra['username'] do
    first_name extra['first_name']
    last_name  extra['last_name']
    email      extra['email']
    password(lazy { extra['password'] || vault_fetch(creds['path'], creds['password_field']) })
    public_key(lazy { vault_fetch(creds['path'], creds['public_key_field']) })
    public_key_path extra['public_key_path']
    org             bootstrap['org']['short_name']
  end
end
