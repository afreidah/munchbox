...
name         = "prowlarr"
type         = "service"
image        = "linuxserver/prowlarr:2.6.2-nightly"
port         = 9696
static_port  = 9696
host_network = true
size         = "medium"
...
tags = [
  ...
  "media",
  "arr",
  "traefik.http.routers.prowlarr.middlewares=oauth2-proxy-errors@file,oauth2-proxy@file",
  # HTTP router for CF tunnel
  "traefik.http.routers.prowlarr-http.rule=Host(`prowlarr.munchbox.cc`)",
  "traefik.http.routers.prowlarr-http.entrypoints=web",
  "traefik.http.routers.prowlarr-http.middlewares=cf-tunnel-https@file,oauth2-proxy-errors@file,oauth2-proxy@file"
]
