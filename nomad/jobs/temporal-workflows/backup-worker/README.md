...
snapshots, Consul snapshots, PostgreSQL dumps from the Patroni
cluster, and a tarball of the container registry. Local copies land
on `/mnt/gdrive`; off-site copies go to s3-orchestrator. Schedules
are owned by the `temporal-backup-trigger` periodic job.

## Image

`registry.munchbox.cc/backup-worker:v0.1.4`
...
- Memory pinned at 512 MiB even though steady-state is ~30 MiB;
  Postgres + registry dumps OOM at the smaller limit
