        ...
        data        = <<-EOF
{
  ...
  "S3PublishEndpoints": {
    "munchbox": {
      ...
      "awsAccessKeyID": "{{ with secret "secret/data/s3-identity/aptly" }}{{ .Data.data.access_key }}{{ end }}",
      "awsSecretAccessKey": "{{ with secret "secret/data/s3-identity/aptly" }}{{ .Data.data.secret_key }}{{ end }}",
